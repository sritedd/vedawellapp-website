...
>
> **HARDENING REVIEW COMPLETE** (2026-04-17 → 2026-04-21, 13 sessions): All 10 phases DONE. Ship verdict **GREEN — fully cleared**. All migrations v36–v43 verified applied in prod via `pg_catalog` probe. Reference → `guide/14-FULL-APP-REVIEW.md`. Post-launch backlog lives in Phase 10 §10.2.

## Session log — full-review
- 2026-09-09/11 — **Phase 0 of guide/19 built, plus much of Phase 1** (fdb81ca → fc69a82, ~20 commits, each verified on prod). Guardian owns its chrome and `<title>`, achievements off; one `LegalNotice` + `/guardian/legal-notice`; `lastVerified`/`source` on every state rule via `VerifiedAt`, with four corrections (SA $20k, VIC Home Warranty from 1 Jul 2026, TAS scheme/citation, **ACT has no cooling-off** — the app had said five days); five sections re-cut around **Pay** with owner vocabulary (E2E maps updated in lockstep); onboarding → four outcomes; the wizard asks "Where is the build now?" and ShouldIPay shows a pre-flight checklist instead of "DO NOT PAY" on day one, skipping claims for completed stages and the deposit without inventing paid rows; pricing tells the truth; member roles honest (read-only); `RightsOnSite` tab; "More" grouped; AI-busy copy that names the deterministic fallback; claim-due reminder emails (the nudge behind the beta's one metric); `scripts/check-gov-links.mjs`, `scripts/verify-migrations.mjs`, `guide/LEGAL-REVIEW-CHECKLIST.md`, a 375px loop test, nightly prod E2E and PR-checklist rules. Verification: **104/104 passed, no failures, no flakes** (38 min, 2026-09-11) — the 8-state workflow, AI and no-fake-data suites against the live deploy. The first fully clean run of this suite. **The migration probe found `schema_v52` unrun — prod still caps free defects at 3 while the pricing page says unlimited; run it before the beta. It also found `migraine_logs` already exists, so the long-standing "v46 pending" note was wrong.**
- 2026-09-08/09 — **Live-prod E2E backlog worked to the end** (`guide/18-BACKLOG.md`: 13 done · 4 open · 1 partial; commits 50effe4…85d10fc). Three items previously believed done were not, and each hid the same way — green type-checks, plausible code, never driven on prod: (1) **B-5c** — `projects.state` defaults to 'NSW' and the E2E seed never set it, so the "8-state" suite created NSW projects eight times over and reported 70/72 green while every `project.state` branch ran as NSW (the VIC/QLD/WA cases of the licence-link switch had never executed once). Fixed 7804a0a; the spec now asserts the stored column AND a state-dependent branch rendering, proven by disabling the fix and watching it fail. (2) **B-9** — the storage-sweep route had no `netlify/functions/cron-*.mts`, so it had never run; fixed 50effe4, now registered on Netlify (`0 16 * * 6`), dry-run verified on prod. (3) **B-13** — every licence-register link in all 8 states was dead or wrong (five 404s, NSW/WA redirecting to department landing pages, SA on the planning portal, VBA → Building and Plumbing Commission), across THREE duplicate maps; fixed 4f81123 — one map in `calculations.ts`, the 8 insurance-check links, 24 more rotten URLs in the workflow JSON / DisputeResolution / blog, all verified with a browser UA or a real Chrome session for WAF-fronted sites. Also: two NSW workflow failures were harness bugs (helpers returned false and all 13 callers ignored it — now throw); `cleanupE2EProjects()` scoped per state; Jest was running the whole suite twice via a stray worktree in `.claude/worktrees/` (ignore pattern added; worktree left for owner — B-15). **Verification on the live deploy (85d10fc):** 8-state workflow suite 79 passed · 1 flaky of 80 (31 min; the flake was SA 'Stage Gate' section-click timing under load — 3/3 in isolation); AI spec AI + no-fake-data together: 23 passed · 1 skipped of 24. **Commercial state (read-only probes 2026-09-09):** 6 profiles (2 admin = owner, 2 E2E, 1 test, 1 owner-personal) → zero external users; Stripe live: 1 customer, 1 subscription **paused since 2026-02-26**; 0 webhook events; email_subscribers 0; page_views are the E2E suite only. AI operational (Gemini flash-lite; intermittent "high demand" retries land on the 503 fallback). **Owner queue:** B-12 yearly Stripe price; B-15 remove worktree; `schema_v46_migraine_logs.sql` still not run; uncommitted Lemonade local-AI WIP in `provider.ts` / `ai/chat/route.ts` / `.env.example` left untouched. Open engineering: B-10 OCR (large), B-14 gov-link check script, B-2 schema regen needs a real `pg_dump`.
- 2026-07-14 — **Business-flow audit + fixes.** Audited tiers/trial/referral/Stripe/quotas against guide vision. Fixed: (1) `checkProAccess` now returns `tier:"admin"` for admins — previously the admin quota pool was unreachable from 5 routes and admin+free accounts got chat limit 0 (hard 429 on every message); (2) Builder Check scrubbed from ALL marketing surfaces (guardian landing JSON-LD offer/featureList/card, homepage ×2, pricing meta+bullets, AILaunchBanner, blog CTA, AI announcement post) — replaced with shipped Claim Review AI; API stays 503-disabled; (3) `invoice.paid` now retrieves the live subscription and verifies status+Guardian-Pro price before re-upgrading (out-of-order event protection); `subscription.updated` also price-checked; (4) subscription cancel / payment-failed downgrades preserve a still-valid trial (`downgradeTier()` → "trial" not "free" when trial_ends_at future); (5) `checkDailyQuota` counts only `success=true` rows; (6) GA4 fallback 990→1499 cents; (7) cleanup-trials comment GET→POST. Verified v44/v45 applied in prod via PostgREST column probe. Docs synced: migrations list (v21–v45 all applied), chat quota pools + lifetime preview documented in CLAUDE.md + 06-USER-WORKFLOW, Builder Check marked DISABLED in 06.
- 2026-07-13 (evening) — **Save-as-PDF export added** to Jyotisha tool (commit `ecf8898`). Print-to-PDF pipeline (no PDF lib — CSP-safe, vector text): print stylesheet with light-paper token overrides, dark canvases kept as figures via `print-color-adjust:exact`, controls hidden, break-inside rules, print-only meta header; beforeprint/afterprint + button expand all MD accordions & details then restore. Kutumba family timeline (`51afa4d`) verified live on vedawellapp.com same day.
- 2026-07-13 (later still) — **Kutumba family timeline added** to Jyotisha tool. `familyConfluence(members)` in engine finds calendar windows where ≥2 of up to 4 family charts simultaneously run supportive (≥58) or testing (<42) bhuktis (interval intersection, member-set merge, ≥90-day filter). UI: cast each chart → "+ Add current chart"; grade-band strips per member on one shared calendar axis (canvas, hover tooltip, now-line), legend, supportive/testing confluence lists with per-member MD–AD detail; profiles persist in localStorage only. Deliberately NOT a bar graph: scores are self-relative per chart, so rows compare timing, never magnitude — disclaimer in UI. Tests 115→125 (synthetic overlap/merge/filter cases + 2-real-chart span/determinism); smoke harness now drives the 2-member flow end-to-end. Artifact + public/jyotish.html redeployed.
- 2026-07-13 (later) — **Dasha-bhukti life engine added** to Jyotisha tool. `lifeTimeline(chart, 90)` in `guide/astro/engine/engine.js` scores every MD/AD window birth→90 by classical Parashari rules (BPHS functional lordship per lagna incl. yogakaraka/kendradhipati, dignity/placement/combustion/retro, ashtakavarga bindus, uccha bala, MD→AD sign-count + naisargika maitri, own-bhukti & dasha-chidra) and generates rule-traceable narrative per bhukti. UI: "Jeevana Rekha" section — canvas life-quality curve (hover tooltip, now-marker, MD bands) + per-MD accordion with graded bhukti rows + peak/care overview chips. Tests now 115/115 (added lordship/yogakaraka tables, timeline contiguity birth→90, determinism, chidra/own flags, score bounds). Artifact republished same URL; `public/jyotish.html` updated. Scoring is a ranking of the chart's own periods (0–100), disclaimed as tradition-language tendencies; no death/maraka predictions by design.
- 2026-07-13 — **Jyotisha Engine shipped into the app** (commit `97b60f9`, pushed → Netlify auto-deploy). Live at `/tools/vedic-birth-chart` (launcher page: catalog + metadata + JSON-LD; sitemap auto-discovers) → opens `/jyotish.html` (self-contained static app in `public/`). Kept global CSP untouched: iframe embed was impossible (`frame-ancestors 'none'` + `X-Frame-Options DENY` in BOTH next.config.ts and netlify.toml — Netlify concatenates duplicate CSP headers, strictest wins), so it launches as a full page instead. Font served from `/fonts/rozha-jyotish.woff2` because CSP `font-src 'self'` blocks data-URI fonts. Regenerate both flavors with `node guide/astro/engine/build.mjs` (artifact + standalone; generated outputs gitignored). tsc + `npm run build` green. Unrelated dirty files (.env.example, ai/chat route, ai/provider) left uncommitted — pre-existing local edits, not mine.
- 2026-07-12 — **Jyotisha Engine built** (standalone astrology tool from `guide/astro/` spec). JS port of `jyotish_core.py` upgraded to spec Tier B: planets+Sun on truncated VSOP87D (generated from IMCCE files via `guide/astro/engine/gen-vsop.mjs`), Moon on Meeus ch.47. Validated vs JPL DE421 (Skyfield, 1,200 epochs 1900–2050): Sun/planets ≤0.02′, Moon ≤0.22′; 88/88 spec checks (Meeus examples, eclipse new-moon roots ±35 s, sunrise ±2 s, Spica@180°, dasha invariants, SAV=337). Phase-2 items done: sunrise-anchored panchanga **with root-found end times**, uccha/cheshta bala, sputa drishti, full BPHS ashtakavarga. NOT done via pyswisseph — **pip install fails on this machine (no C compiler)**; native VSOP87D replaces it at equal/better accuracy. Deliverables: self-contained `guide/astro/jyotish_tool.html` (also published as Claude artifact), reproducible pipeline in `guide/astro/engine/`. Note: this engine can later back the Panchang rebuild (roadmap item) — tithi/yoga/karana/nakshatra + end times are already computed.
- 2026-04-21 (session 14) — Migration verification sweep. Ran read-only `pg_catalog` query covering v36–v43. First pass: 15/16 ✅, 1 ❌ (`enforce_free_defect_limit()` missing — earlier session-9 entry claiming v42 applied was wrong; the SQL was never actually executed). User then ran `schema_v42_defect_limit.sql` in Supabase SQL Editor → all 16 checks now ✅. Free-tier defect cap now enforced server-side (matching v41 variation pattern). `schema_unified.sql` remains stale (missing v36–v42 changes) — separate cleanup task, not blocking. **Trust-but-verify lesson**: never mark a migration "applied" based on a "please run this" instruction — only based on a pg_catalog probe of the live DB.
...
- ✅ `schema_v21–v35` — Phone verification, prehandover, realtime, contract/builder reviews, email verification, security hardening, identity_verified, defect SLA, account deletion, ai_conversations, activity log, escalations, project members, sprint4, SaaS infra (ALL APPLIED)
- ✅ `schema_v36–v43` — AI telemetry, page views, social tracking, RLS fix, member access, variation/defect limits, FK cleanup (ALL APPLIED — verified 2026-04-21 via pg_catalog probe, 16/16 checks)
- ✅ `schema_v44_lead_nurture.sql` + `schema_v45_lead_utm.sql` — email_subscribers nurture/UTM columns (APPLIED — verified 2026-07-14 via PostgREST column probe: all 6 columns selectable)
- ⚠️ `schema_unified.sql` is STALE (missing v36+ changes) — prod is ahead of unified; separate cleanup task
...
### Remaining Gaps
- **2026-09-09**: `guide/18-BACKLOG.md` is the live queue (13 done · 4 open · 1 partial). Open: B-10 OCR for scanned PDFs, B-12 yearly Stripe price (owner), B-14 gov-link rot backstop + 3 generic complaint links, B-15 stray worktree (owner). Lesson recorded there: assert the discriminator column, not the label; a cron route without a schedule is dead code; government links rot every ~2 years.
